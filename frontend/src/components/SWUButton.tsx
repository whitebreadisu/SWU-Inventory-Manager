...
export function SWUButton({ children, active = true, onClick, size = 'sm' }: Props) {
  ...
  const rightTipX    = 68.40 + OFFSET *  0.8;
  ...
  return (
    <button type="button" onClick={onClick} className="swu-btn" style={{ height: h }}>
      ...
      >
        ...
        <polyline fill="none" stroke={edge} strokeWidth="2"
      ...
      <span className="swu-btn__center">
        <svg className="swu-btn__center-bg"
             viewBox="0 0 100 150.43"
             preserveAspectRatio="none"
             aria-hidden="true">
          <rect x="0" y={MAIN_TOP} width="100" height={MAIN_BOTTOM - MAIN_TOP} fill={fill} />
          <line x1="0" y1={MAIN_TOP}    x2="100" y2={MAIN_TOP}
                stroke={edge} strokeWidth="2" vectorEffect="non-scaling-stroke" />
          <line x1="0" y1={MAIN_BOTTOM} x2="100" y2={MAIN_BOTTOM}
                stroke={edge} strokeWidth="2" vectorEffect="non-scaling-stroke" />
