...
vi.mock('../../api/inventory', () => ({
  incrementCard: vi.fn().mockResolvedValue({
    card_id: 1, quantity: 1, playset_complete: false, blocked: false, reason: null,
  }),
}));
...
function makeCard(overrides: Partial<CardWithQty>): CardWithQty {
  return {
    ...
    name: 'IG-88', rarity: 'R', type: 'Unit', is_foil: false, is_hyperspace: false,
    is_prestige: false, is_showcase: false, is_organized_play: false,
    aspects: [], keywords: [], traits: [], cost: 3, power: 4, hp: 4, arena: 'Ground',
    quantity: 0,
    ...overrides,
  };
}
...
describe('AddCardsModal', () => {
  ...
  it('splitForVerification: willAdd vs willSkip reflected in verification view', async () => {
    ...
    const limitedCatalog: CardWithQty[] = [
      makeCard({ id: 10, set_code: 'JTL', base_card_number: '12', card_number: '12', type: 'Unit', quantity: 3 }),
      makeCard({ id: 11, set_code: 'JTL', base_card_number: '55', card_number: '55', type: 'Unit', quantity: 0 }),
    ];
    await act(async () => {
      render(
        <AddCardsModal
    ...
    await act(async () => { fireEvent.submit(form2); });
