...
const sorCapCity      = makeCard({ id: 1, set_code: 'SOR', base_card_number: '20',  card_number: '20',  type: 'Base',    name: 'Capital City',                                quantity: 0 });
const sorVeersOp      = makeCard({ id: 2, set_code: 'SOR', base_card_number: '230', card_number: '20',  type: 'Unit',    name: 'General Veers - Blizzard Force Commander',    is_organized_play: true,  quantity: 0 });
const sorVeersOpFoil  = makeCard({ id: 3, set_code: 'SOR', base_card_number: '230', card_number: '20',  type: 'Unit',    name: 'General Veers - Blizzard Force Commander',    is_organized_play: true,  is_foil: true, quantity: 0 });

// SOR #286 — Capital City Hyperspace (single non-OP, no OP at this number)
const sorCapCityHyper = makeCard({ id: 4, set_code: 'SOR', base_card_number: '20',  card_number: '286', type: 'Base',    name: 'Capital City',                                is_hyperspace: true, quantity: 0 });

// SOR #69 — Resilient Standard + Foil (two non-OP share this number)
const sorResilient     = makeCard({ id: 5, set_code: 'SOR', base_card_number: '69',  card_number: '69',  type: 'Upgrade', name: 'Resilient',                                   quantity: 0 });
const sorResilientFoil = makeCard({ id: 6, set_code: 'SOR', base_card_number: '69',  card_number: '69',  type: 'Upgrade', name: 'Resilient',                                   is_foil: true, quantity: 2 });

// SOR #334 — Resilient Hyperspace + Hyperspace Foil (two non-OP share this number)
const sorResilientHyper     = makeCard({ id: 7, set_code: 'SOR', base_card_number: '69', card_number: '334', type: 'Upgrade', name: 'Resilient', is_hyperspace: true, quantity: 0 });
const sorResilientHyperFoil = makeCard({ id: 8, set_code: 'SOR', base_card_number: '69', card_number: '334', type: 'Upgrade', name: 'Resilient', is_hyperspace: true, is_foil: true, quantity: 0 });
...
const secStandard   = makeCard({ id: 20, set_code: 'SEC', base_card_number: '8', card_number: '8',   type: 'Leader', name: 'Bail Organa - Doing Everything He Can', quantity: 0 });
const secHyperspace = makeCard({ id: 21, set_code: 'SEC', base_card_number: '8', card_number: '272', type: 'Leader', name: 'Bail Organa - Doing Everything He Can', is_hyperspace: true, quantity: 0 });

const catalog: CardWithQty[] = [
  sorCapCity, sorVeersOp, sorVeersOpFoil,
  sorCapCityHyper,
  sorResilient, sorResilientFoil,
  sorResilientHyper, sorResilientHyperFoil,
  sorLeader,
  secStandard, secHyperspace,
];
...
describe('resolveRow', () => {
  ...
  it('returns needs_variant for OP when multiple OP cards share the card_number', () => {
    const result = resolveRow('SOR', makeRow({ cardNumber: '20', op: true, variant: null }), catalog);
  ...
  it('resolves to the chosen OP variant when variant is picked', () => {
    const result = resolveRow('SOR', makeRow({ cardNumber: '20', op: true, variant: 'Foil' }), catalog);
    expect(result.status).toBe('resolved');
    if (result.status === 'resolved') {
