...
| Slice 5 | S5 | *✅ Effectively delivered 2026-06-21* via the catalog redesign, not the standalone backfill-slice path described here: `front_image_url`/`back_image_url` are populated per `card_variants` row by the swuapi ingestion (BL-29) and surfaced in the card detail / card inventory popups (S6). See `SWU_Catalog_Redesign_Spec.md`; full reconciliation of this spec tracked in BL-47. |
| Slice 6 | S6 | *✅ Implemented & deployed to prod 2026-06-21* as part of the catalog redesign frontend rewire (not the standalone-slice path planned here). Clicking a Catalog/Inventory card name opens a modal with the variant-switchable image (front/back flip on leaders), stacked variant buttons consolidated by `stamp_group` (BL-31), and full card metadata — built against `base_cards`/`card_variants` via `GET /api/base-cards/{id}`. Authoritative as-built spec: `SWU_Catalog_Redesign_Spec.md` §5.3. (This row + §9.2 predate the redesign; full reconciliation tracked in BL-47.) |
...
**✅ Implemented & deployed to prod 2026-06-21** as part of the catalog redesign frontend rewire — built against `base_cards`/`card_variants` via `GET /api/base-cards/{id}`, with `stamp_group` consolidation (BL-31). The authoritative as-built spec is `SWU_Catalog_Redesign_Spec.md` §5.3; the requirements below are retained as historical context and predate the redesign (full reconciliation of this document is tracked in BL-47).
