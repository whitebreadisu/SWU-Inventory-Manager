...
**P7 completed 2026-06-14** — all four stages (Dependabot, concurrency-safe upserts, expanded RLS/coverage tests, OWASP/secrets review) done; see the Phase Table and Open Decisions below. The platform has reached that stable, audited state — feature work (S5, Decks) is unpaused as of this point.
...
- **P6 — Error tracking provider. ✅ Resolved 2026-06-14.** Cloud Error Reporting selected over Sentry — GCP-native, requires no new account/SDK/tool: Cloud Run's stdout/stderr already flows to Cloud Logging, and Error Reporting auto-detects entries with `severity=ERROR` and a stack trace, grouping them automatically. Consistent with the "smoothest onramp" GCP-first reasoning from P1/P5. A full comparison against Sentry (richer grouping/source-context/alerting UX, at the cost of a separate third-party account) is recorded in `SWU_Platform_Learning_Guide.md`'s P6 chapter for later review.
- **P7 — Scope of concurrency-safe inventory updates. ✅ Resolved 2026-06-14.** A dedicated P7 stage (Stage 2) — not folded into the testing-expansion stage, and not patched ad hoc outside P7. `app/repositories/inventory.py`'s `upsert_increment`/`upsert_decrement` use a SELECT-then-mutate-then-commit pattern — a lost-update race under concurrent requests for the same `(tenant_id, card_id)` — and the frontend fires one request per click with no in-flight guard, so a slower click's response can overwrite a faster click's more recent state. Both halves get a deliberate fix and a dedicated regression test. Full comparison against the two alternatives recorded in `SWU_Platform_Learning_Guide.md`'s P7 chapter for later review.
- **P7 — Dependabot PR backlog: triage now vs. defer. ✅ Resolved 2026-06-14.** Document the current state and defer triage to a dedicated future session. Stage 4 planning anticipated reviewing 7 Dependabot alerts plus 1 major-version PR; by Stage 4, the weekly schedule (Stage 1) had produced 18 open PRs (13 passing, 5 failing on major-version bumps), none touching a library with an open security alert. Recorded in `specification_documents/SWU_Platform_Security_Review.md` (A06) with notes for that session — check #11/#12 (multi-package bumps) for overlap with #21/#24 before merging, then investigate the 5 failing major-version PRs individually. Full comparison recorded in `SWU_Platform_Learning_Guide.md`'s P7 chapter for later review.
